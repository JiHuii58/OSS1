...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post("http://localhost:8081/users/forgot-password", { email });
  ...
  return (
    <div className="forgot_password">
      ...
      <style jsx>{`
        ...
        .forgot_password input:hover:not(:focus) {
          border-color: #a3bffa;
        }
        ...
        .forgot_password button:hover::before {
          width: 300px;
          height: 300px;
        }
      `}</style>
